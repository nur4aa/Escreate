  ...
  async (req, res) => {
    ...
    try {
      ...
      board.activity.unshift({
        text: `${user.name} добавил '${title}' to '${list.title}'`,
      });
      await board.save();
...
router.patch("/edit/:id", [auth, member], async (req, res) => {
  try {
    const { title, description, label, date } = req.body;
    if (title === "") {
      return res.status(400).json({ msg: "Название обязательно" });
    }
...
router.patch("/archive/:archive/:id", [auth, member], async (req, res) => {
  try {
    ...
    board.activity.unshift({
      text: card.archived
        ? `${user.name} заархивировал '${card.title}'`
        : `${user.name} переместил карту'${card.title}' в доску`,
    });
    await board.save();
...
router.patch("/move/:id", [auth, member], async (req, res) => {
  try {
    ...
    if (fromId !== toId) {
      ...
      board.activity.unshift({
        text: `${user.name} переместил '${card.title}' из '${from.title}' в '${to.title}'`,
      });
      await board.save();
    }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const board = await Board.findById(req.header("boardId"));
      board.activity.unshift({
        text: `${user.name} ${add ? "вступил" : "вышел"} '${card.title}'`,
      });
      await board.save();
...
router.delete("/:listId/:id", [auth, member], async (req, res) => {
  try {
    ...
    const board = await Board.findById(req.header("boardId"));
    board.activity.unshift({
      text: `${user.name} удален '${card.title}' из '${list.title}'`,
    });
    await board.save();
